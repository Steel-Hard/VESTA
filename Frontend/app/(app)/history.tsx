...
export default function FallAlert() {
  const [alertData, setAlertData] = useState<LastFallAlertResponse | null>(
    null
  );
  ...
  const handleMarkResolved = async () => {
    ...
    if (!alertData.deviceId) {
      ...
      Alert.alert(
        "Erro",
        "Não foi possível identificar o dispositivo para resolver o alerta."
      );
      return;
    }
    ...
    if (!alertId) {
      ...
      Alert.alert(
        "Erro",
        "Não foi possível identificar o alerta para resolver."
      );
      return;
    }
    ...
    try {
      setIsResolving(true);
      await DeviceService.markAlertAsResolved(
        alertData.deviceId,
        alertData.metric?._id
      );
  ...
  return (
    ...
    >
      ...
      <View>
        ...
        <Text style={styles.subtitle}>
          Queda Detectada
          {alertData.elderName ? ` - ${alertData.elderName}` : ""}
        </Text>
        ...
        {alertData.metric && (
          <View style={styles.card}>
            ...
            <Text style={styles.cardHour}>
              X: {alertData.metric.x.toFixed(2)} | Y:{" "}
              {alertData.metric.y.toFixed(2)} | Z:{" "}
              {alertData.metric.z.toFixed(2)}
            </Text>
        ...
        <Pressable
          style={[
            styles.button,
            styles.buttonGreen,
            alertData.metric?.isResolved || isResolving ? { opacity: 0.6 } : {},
          ]}
